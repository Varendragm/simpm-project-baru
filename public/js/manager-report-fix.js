...
function makeReportHtml(d){
 ...
 h+=r.map(function(x){return '<tr><td>'+esc(x.machine)+'</td><td>'+esc(x.station)+'</td><td class="r">'+tableN(x.oee)+'%</td><td class="r">'+tableN(x.availability)+'%</td><td class="r">'+tableN(x.performance)+'%</td><td class="r">'+tableN(x.quality)+'%</td><td class="r">'+tableN(x.reliability)+'%</td><td class="r">'+tableN(x.mttr)+'</td><td class="r">'+tableN(x.mtbf)+'</td><td class="r">'+tableN(x.downtime)+' jam</td><td class="cond">'+esc(x.kondisi||'Belum ada data')+'</td></tr>';}).join('');
 h+='</tbody></table></div>';}
...
window.exportManagerExcel=function(){
 ...
 rows().then(function(d){
  var r=d.rows||[];
  var h='<!doctype html><html><head><meta charset="utf-8"><style>body{font-family:Arial}h2{color:#0b7285}table{border-collapse:collapse;width:100%}th{background:#0b7285;color:#fff;border:1px solid #777;padding:7px}td{border:1px solid #aaa;padding:6px}.num{text-align:right}.notice{padding:8px;background:#fff9db;border:1px solid #f1c40f}</style></head><body><h2>SIMPM · PG RENDENG</h2><h3>'+esc(title(d))+'</h3><p>Periode: '+esc(period(d))+' · '+r.length+' mesin</p>';
  ...
  h+='<table><tr><th>Mesin</th><th>Stasiun</th><th>OEE</th><th>Availability</th><th>Performance</th><th>Quality</th><th>Reliability</th><th>MTTR (jam)</th><th>MTBF (jam)</th><th>Downtime (jam)</th><th>Kondisi</th></tr>';
  h+=r.map(function(x){return '<tr><td>'+esc(x.machine)+'</td><td>'+esc(x.station)+'</td><td class="num">'+tableN(x.oee)+'%</td><td class="num">'+tableN(x.availability)+'%</td><td class="num">'+tableN(x.performance)+'%</td><td class="num">'+tableN(x.quality)+'%</td><td class="num">'+tableN(x.reliability)+'%</td><td class="num">'+tableN(x.mttr)+'</td><td class="num">'+tableN(x.mtbf)+'</td><td class="num">'+tableN(x.downtime)+'</td><td>'+esc(x.kondisi||'Belum ada data')+'</td></tr>';}).join('');
  h+='</table></body></html>';
