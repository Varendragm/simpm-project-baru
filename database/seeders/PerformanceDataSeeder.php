<?php

namespace Database\Seeders;

use App\Models\Machine;
use App\Models\MachineProductionRecord;
use Carbon\Carbon;
use Illuminate\Database\Seeder;

class PerformanceDataSeeder extends Seeder
{
    public function run(): void
    {
        Machine::query()->update(['status' => 'aktif']);

        // Dummy production data for the latest six months, including the
        // current reporting month. The calculator remains fully formula-based.
        $months = [];
        $reference = Carbon::now()->startOfMonth();
        for ($i = 5; $i >= 0; $i--) {
            $start = $reference->copy()->subMonths($i)->startOfMonth();
            $end = $start->copy()->endOfMonth();
            $months[] = [
                'start' => $start->toDateString(),
                'end' => $end->toDateString(),
                'days' => $start->daysInMonth,
            ];
        }

        $machines = Machine::pluck('id')->all();

        // [actual, ideal, good] demo production capacity per 31-day month.
        $base = [
            'm-g01' => [40320, 43200, 38000],
            'm-g02' => [35000, 43200, 33000],
            'm-g03' => [41400, 43200, 40500],
            'm-g04' => [36000, 43200, 34200],
            'm-b01' => [39600, 43200, 37800],
            'm-b02' => [38800, 43200, 37000],
            'm-b03' => [36500, 43200, 34500],
            'm-p01' => [41000, 43200, 39500],
            'm-p02' => [41500, 43200, 40200],
        ];

        $variation = [
            [1.00, 1.00, 1.00],
            [0.98, 1.00, 0.99],
            [1.02, 1.00, 1.01],
            [0.96, 1.00, 0.98],
            [1.01, 1.00, 1.00],
            [1.00, 1.00, 1.00],
        ];

        foreach ($machines as $machineId) {
            $baseValues = $base[$machineId] ?? [38000, 43200, 36500];

            foreach ($months as $index => $month) {
                $days = $month['days'];
                $planned = $days * 24 * 60;
                $ideal = $baseValues[1] / 31 * $days;
                $actual = $baseValues[0] / 31 * $days * $variation[$index][0];
                $good = $baseValues[2] / 31 * $days * $variation[$index][2];

                MachineProductionRecord::updateOrCreate(
                    [
                        'machine_id' => $machineId,
                        'period_start' => $month['start'],
                        'period_end' => $month['end'],
                    ],
                    [
                        'planned_minutes' => $planned,
                        'actual_output' => round($actual, 2),
                        'ideal_output' => round($ideal, 2),
                        'good_output' => round(min($actual, $good), 2),
                    ]
                );
            }
        }
    }
}
